...
def extract_and_load_product():
    ...
    try:
        ...
        print(f"Extracted {len(rows)} product records from SQL Server")
        ...
        postgres_cursor.execute("""
        TRUNCATE TABLE "DimProduct" RESTART IDENTITY;
        """)
        ...
        print(
            f"Successfully loaded {len(rows)} records into DimProduct"
        )
...
with DAG(
    ...
    load_dim_product = PythonOperator(
        task_id="extract_and_load_dim_product",
        python_callable=extract_and_load_product,
    )